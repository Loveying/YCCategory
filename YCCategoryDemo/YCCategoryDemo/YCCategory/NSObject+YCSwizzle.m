...
@implementation NSObject (YCSwizzle)
    ...
    if (!directOriginalMethod || !directAlternateMethod) {
        ...
        if (!directOriginalMethod) {
            ...
            if (!inheritedOriginalMethod) {
                SetNSError(error_, @"original method %@ not found for class %@", NSStringFromSelector(origSel), [self className]);
                return NO;
            }
        }
        if (!directAlternateMethod) {
            ...
            if (!inheritedAlternateMethod) {
                SetNSError(error_, @"alternate method %@ not found for class %@", NSStringFromSelector(altSel), [self className]);
                return NO;
            }
        }
...
+ (BOOL)yc_swizzleClassMethod:(SEL)origSel withMethod:(SEL)altSel error:(NSError**)error {
    return [GetClass((id)self) yc_swizzleMethod:origSel withMethod:altSel error:error];
}
